...
このドキュメントでは、Streamlit学習アプリを一から構築する手順を説明します。
...
3. [Step 1: メインアプリの作成](#3-step-1-メインアプリの作成)
4. [Step 2: 基本コンポーネントページの作成](#4-step-2-基本コンポーネントページの作成)
5. [Step 3: レイアウトページの作成](#5-step-3-レイアウトページの作成)
6. [Step 4: データ表示ページの作成](#6-step-4-データ表示ページの作成)
7. [Step 5: スクレイパーの作成](#7-step-5-スクレイパーの作成)
8. [Step 6: スクレイピング可視化ページの作成](#8-step-6-スクレイピング可視化ページの作成)
9. [動作確認](#9-動作確認)
...
ターミナル（コマンドプロンプト）を開いて、Pythonがインストールされているか確認します。
...
# プロジェクト用ディレクトリを作成
mkdir streamlit_learning
cd streamlit_learning
```

### 1.4 仮想環境の作成（推奨）

仮想環境を使うと、プロジェクトごとにパッケージを管理できます。
...
インストール確認：
```bash
pip list | grep -E "streamlit|selenium|pandas|plotly"
```

### 1.6 Google Chromeのインストール確認

スクレイピング機能を使う場合、Google Chromeが必要です。
...
`app.py` を作成します。これがアプリのエントリーポイントになります。

```bash
touch app.py
```

エディタで `app.py` を開き、以下のコードを入力します。
...
- 学習の流れ（Step 1〜4）が表示される

`Ctrl + C` でサーバーを停止します。

---

## 4. Step 2: 基本コンポーネントページの作成

`pages/01_基本コンポーネント.py` を作成します。

```bash
touch "pages/01_基本コンポーネント.py"
```

以下のコードを入力します（長いので、セクションごとに説明します）。
...
サイドバーから「基本コンポーネント」を選択して、各コンポーネントが動作することを確認します。

---

## 5. Step 3: レイアウトページの作成

`pages/02_レイアウト.py` を作成します。

```bash
touch "pages/02_レイアウト.py"
```
...
## 6. Step 4: データ表示ページの作成

`pages/03_データ表示.py` を作成します。

```bash
touch "pages/03_データ表示.py"
```
...
`scraper/nikkei_scraper.py` を作成します。

```bash
touch scraper/nikkei_scraper.py
```
...
## 8. Step 6: スクレイピング可視化ページの作成

`pages/04_スクレイピング可視化.py` を作成します。

```bash
touch "pages/04_スクレイピング可視化.py"
```
...
1. **トップページ**: 学習の流れが表示される
2. **基本コンポーネント**: ボタン、入力、スライダーが動作する
3. **レイアウト**: カラム、タブが正しく表示される
4. **データ表示**: グラフとフィルタリングが動作する
5. **スクレイピング可視化**: データが取得できる（またはモックデータが表示される）
...
このハンズオンを通じて、Streamlitの基本からスクレイピング可視化まで学ぶことができました。
ぜひ自分のプロジェクトでStreamlitを活用してください！
